# Release notes — v1.0.0 proposed proof

This release presents an ordinary proposed proof of planar strictly convex
hyperrigidity for compact convex sets of affine dimension at most two.

## Signed-off artifacts

- TeX:
  `paper/planar_strict_convex_hyperrigidity.tex`
  — SHA-256
  `1902dc564a0a2fc09cd6a04fa6d56943deae15453b79d9841a3998b462426014`.
- PDF:
  `paper/planar_strict_convex_hyperrigidity.pdf`
  — SHA-256
  `27ffab5e1a5d3ab99f2bbe4f5b44d6c62b4d7d79aa8860627e1a73f5e2edadb1`.
- Frozen evidence archive:
  `release/planar-strict-convex-hyperrigidity-final-evidence-v4.zip`
  — SHA-256
  `473c8331c57a20431aade7cb54d0048b9bc7db4f7c3cda18a52edef3e471db66`.
- Final release-signoff report:
  `audits/final_corrective_release_signoff_v4_pass.md`
  — SHA-256
  `6493294455f3a7b2aba169475e85e6d9114f1ec84bfad74c1d5a166c1761f522`.

The fresh six-category AI audit returned `OVERALL PASS`. The evidence
archive was not rebuilt after signoff.

## Claim boundary

The release does not claim human peer review, complete Lean verification,
absolute historical priority, or an unrestricted affine-dimension-three
theorem. The exact Lean status and AI contributions are documented in
`AI_DISCLOSURE.md`, `CLAIMS_MATRIX.md`, and `PROVENANCE.md`.
