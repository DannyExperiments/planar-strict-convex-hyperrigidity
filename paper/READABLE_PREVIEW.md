...
> Source SHA-256: `8bf933d2ff955c669b455693102c02d0a08f3326cedd39e57c421d4836edc5e7`
...
No smoothness of \(g\), finite-rank assumption, separability assumption, or
atomicity of the spectral measure is required.
...
functions, by Shankar's normal-generator theorem [Shankar], and radial
power or related monomial systems covered by Pietrzycki--Stochel
[PietrzyckiStochel].  Davidson--Kennedy [DavidsonKennedy] give the
general relationship among Choquet order, unique extension, and
hyperrigidity for function systems.
...
\(Mr^{-d}\) half-open Borel cubes of diameter \(r\).  Pulling the partition
back through \(U\) and applying Cauchy--Schwarz gives, with
\(m=\mu(C)>0\),
...
The evaluation map \(\nu\mapsto\nu(S)\) is Borel for Borel \(S\), and scalar
monotone convergence shows that \(\widehat G\) is a POVM.  For every bounded
Borel \(h\),
...
standard equivalence with hyperrigidity is recorded, for example, in
...
The ordinary proof underlying this manuscript has been frozen and subjected
to multiple independent AI-assisted hostile audits.  Those audits are not
human peer review.  The theorem has not yet been formalized in Lean, and a
specialist human review remains pending.  Two independent solution-aware
literature searches found the dimension-one case in prior work and located
no prior theorem covering arbitrary continuous strictly convex functions on
arbitrary two-dimensional compact convex sets with diffuse representations.
Repository provenance records identify the preserved source artifacts,
audit reports, literature reports, and cryptographic hashes.
